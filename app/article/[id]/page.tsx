      ...
      <main className="max-w-4xl mx-auto px-6 mt-12">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {relatedArticles.map((related) => (
              <Link key={related.id} href={`/article/${related.id}`} className="group">
                <div className="rounded-3xl overflow-hidden mb-4 border border-transparent group-hover:border-red-900 transition-colors">
                ...
                <p className="font-semibold leading-tight text-lg group-hover:text-red-600 transition-colors line-clamp-3 uppercase italic">{related.title}</p>
              </Link>
            ))}
          </div>
        </div>
        ...
        <footer className="mt-32">
          <div className="bg-zinc-50 dark:bg-zinc-900 p-14 rounded-[42px] border border-zinc-100 dark:border-zinc-800 relative">
            <h4 className="text-xs font-black uppercase tracking-[1.5px] text-red-600 mb-6">SIGNAL NEWS HUB DISCLAIMER</h4>
            <p className="text-2xl italic text-zinc-600 dark:text-zinc-400 max-w-2xl leading-snug font-serif">
              "Our mission is to bypass the noise and deliver high-fidelity signals. Perspective is leverage."
            </p>
            <div className="mt-14 flex flex-wrap gap-6">
              <Link href="/" className="px-8 py-5 bg-black dark:bg-white text-white dark:text-black rounded-3xl font-black text-sm uppercase tracking-widest hover:scale-105 transition-transform">NEXT BRIEFING</Link>
              <button onClick={() => window.print()} className="px-8 py-5 border-2 border-zinc-300 dark:border-zinc-700 rounded-3xl text-sm font-black uppercase tracking-widest text-zinc-500 hover:text-red-600 transition-colors">PRINT FOR RECORDS</button>
            </div>
          </div>
        </footer>
      </main>
